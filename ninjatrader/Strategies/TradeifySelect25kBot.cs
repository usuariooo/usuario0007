...
{
	// =============================================================================
	// TRADEIFY SELECT 25K — MNQ 5M
	//
	// Instalar: copia este archivo a
	//   Documents\NinjaTrader 8\bin\Custom\Strategies\TradeifySelect25kBot.cs
	...
	public enum TradeifySelectPhase
	{
		Evaluation,
		FundedDaily
	}

	public class TradeifySelect25kBot : Strategy
	{
		private EMA emaMid;
		private EMA emaSlow;
		private ATR atr;
		...
		protected override void OnStateChange()
		{
			...
			{
				Name = "TradeifySelect25kBot";
				Description = "MNQ 5m Tradeify Select 25k: VWAP bounce + OR retest, longs y shorts.";
				...
				Phase = TradeifySelectPhase.Evaluation;
		...
		{
			Print("===== EMBUDO TradeifySelect25kBot =====");
		...
		private void EnforceAccountGuards()
		{
			...
			if (Phase == TradeifySelectPhase.Evaluation)
			{
		...
		private bool ValidateRiskGeometry(int direction)
		{
			...
			double lossLimit = Phase == TradeifySelectPhase.Evaluation ? DailyLossLimit : FundedDailyLossLimit;
		...
		private int GetBaseQty()
		{
			...
			if (dailyPnL <= DailyLossLimit * 0.45)
				qty = 1;
			if (Phase == TradeifySelectPhase.Evaluation && ChallengeLocksActive())
			{
		...
		private void SubmitTrade(SignalSnapshot signal)
		{
			...
			scalpName = (signal.Direction > 0 ? "TF25_S_L_" : "TF25_S_S_") + tag;
			runName = (signal.Direction > 0 ? "TF25_R_L_" : "TF25_R_S_") + tag;
		...
		private void UpdateOpeningRange(DateTime ny)
		{
			...
			{
				Draw.HorizontalLine(this, "TF25_ORH", sessionOrHigh, Brushes.DodgerBlue);
				Draw.HorizontalLine(this, "TF25_ORL", sessionOrLow, Brushes.OrangeRed);
				if (currentVwap > 0)
					Draw.HorizontalLine(this, "TF25_VWAP", currentVwap, Brushes.Goldenrod);
			}
		}
		...
		private string StateFilePath()
		{
			...
			return Path.Combine(live, "TradeifySelect25kBot_days.csv");
		}
		...
		private void DrawStatusPanel(DateTime ny)
		{
			...
			string phase = Phase == TradeifySelectPhase.Evaluation ? "EVAL SELECT 25K" : "FUNDED DAILY";
			...
			string text =
				"TRADEIFY SELECT 25K | " + marketBias + " | " + dir + "\n"
			...
			Draw.TextFixed(this, "TF25_Panel", text, TextPosition.BottomRight, c,
		...
		public TradeifySelectPhase Phase { get; set; }
